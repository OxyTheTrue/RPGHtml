...
export class UI {
    ...
    async typeLog(text, color = '#00ff9d') {
    ...
    /** Exibe feedback rápido de "Salvo!" no log ou em um toast. */
    showSaveFeedback() {
        const log = document.getElementById('game-log');
        if (log) {
            const line = document.createElement('div');
    ...
    /** Conteúdo HTML do modal de detalhe da skill. */
    getSkillDetailContent(skill, player) {
        const playerSkill = player.skills.find(ps => ps.id === skill.id);
