...
function createDropItem(type, playerLv) {
    ...
    const hpBase = pot.hpBase || 8;
    const hpPerLv = pot.hpPerLv || 2;
    const manaBase = pot.manaBase || 10;
    const manaPerLv = pot.manaPerLv || 2;
    
    const heal = Math.max(8, hpBase + Math.floor((playerLv || 1) * hpPerLv));
    const manaGain = Math.max(10, manaBase + Math.floor((playerLv || 1) * manaPerLv));
...
export class Game {
    ...
    startNewGameWithDifficulty(difficultyMult) {
        ...
        this._pendingName = null;
        this.ui.reset(); // Resetar estado da UI
    ...
    loadGame() {
        ...
        if (!raw) {
            ...
            this.ui.typeLog('Nenhum jogo salvo encontrado.', 'error');
            return;
        }
        try {
            const data = JSON.parse(raw);
            this.player = Player.fromSave(data.p);
            this.enemy = data.e;
            this.setDifficultyMult(data.difficultyMult || 1);
            this._archerTip = data._archerTip || '';
            ...
            this._npc = data._npc || null;
            this._npcPhase = data._npcPhase || null;
            this._healerFullHp = data._healerFullHp || false;
            this._thiefContestMode = data._thiefContestMode || false;
            this._thiefStolen = data._thiefStolen || null;
    ...
    restartAfterDeath() {
        ...
        // Iniciar novo combate diretamente
        this.spawnMonster();
    }
