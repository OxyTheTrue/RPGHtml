...
export class UI {
    constructor() {
        ...
        this.lastHpPercent = 100; // Armazenar último percentual de HP do player
        this.lastEnemyHpPercent = 100; // Armazenar último percentual de HP do inimigo
        this.lastManaPercent = 100; // Armazenar último percentual de Mana do player
    ...
    /** Atualiza a barra de HP do inimigo com duas camadas e delay na camada cinza */
    updateEnemyHP(enemy) {
        const enemyBar = document.getElementById('enemy-bar');
        const enemyBarBg = document.getElementById('enemy-bar-bg');
        
        if (enemyBar && enemy && typeof enemy.maxHp === 'number' && enemy.maxHp > 0) {
            const currentHpPercent = (Math.max(0, enemy.hp) / enemy.maxHp * 100);
            
            // Inicializar lastEnemyHpPercent na primeira execução
            if (this.lastEnemyHpPercent === null || this.lastEnemyHpPercent === undefined) {
                this.lastEnemyHpPercent = currentHpPercent;
                enemyBar.style.width = currentHpPercent + '%';
                if (enemyBarBg) enemyBarBg.style.width = currentHpPercent + '%';
                return;
            }
            
            // Atualizar barra vermelha imediatamente
            enemyBar.style.width = currentHpPercent + '%';
            
            if (enemyBarBg) {
                // Se o HP diminuiu, aplicar delay na barra cinza
                if (currentHpPercent < this.lastEnemyHpPercent) {
                    setTimeout(() => {
                        enemyBarBg.style.width = currentHpPercent + '%';
                    }, 500);
                } else {
                    // Se o HP aumentou, atualizar imediatamente
                    enemyBarBg.style.width = currentHpPercent + '%';
                }
            }
    ...
    /** Atualiza a barra de Mana com duas camadas e delay na camada branca */
    updateMana(player) {
        const manaBar = document.getElementById('p-mana-bar');
        const manaBarBg = document.getElementById('p-mana-bar-bg');
        
        if (manaBar && player.maxMana > 0) {
            const currentManaPercent = (Math.max(0, player.mana) / player.maxMana * 100);
            
            // Inicializar lastManaPercent na primeira execução
            if (this.lastManaPercent === null || this.lastManaPercent === undefined) {
                this.lastManaPercent = currentManaPercent;
                manaBar.style.width = currentManaPercent + '%';
                if (manaBarBg) manaBarBg.style.width = currentManaPercent + '%';
                return;
            }
            
            // Atualizar barra azul imediatamente
            manaBar.style.width = currentManaPercent + '%';
            
            if (manaBarBg) {
                // Se a Mana diminuiu, aplicar delay na barra branca
                if (currentManaPercent < this.lastManaPercent) {
                    setTimeout(() => {
                        manaBarBg.style.width = currentManaPercent + '%';
                    }, 500);
                } else {
                    // Se a Mana aumentou, atualizar imediatamente
                    manaBarBg.style.width = currentManaPercent + '%';
                }
            }
    ...
    /** Reseta o estado da UI para novo jogo */
    reset() {
        this.lastHpPercent = 100;
        this.lastEnemyHpPercent = 100;
        this.lastManaPercent = 100;
    }

    /** Atualiza a barra de HP com duas camadas e delay na camada vermelha */
    updateHP(player) {
        const hpBar = document.getElementById('p-hp-bar');
        const hpBarBg = document.getElementById('p-hp-bar-bg');
        
        if (hpBar && player.maxHp > 0) {
            const currentHpPercent = (Math.max(0, player.hp) / player.maxHp * 100);
            
            // Inicializar lastHpPercent na primeira execução
            if (this.lastHpPercent === null || this.lastHpPercent === undefined) {
                this.lastHpPercent = currentHpPercent;
                hpBar.style.width = currentHpPercent + '%';
                if (hpBarBg) hpBarBg.style.width = currentHpPercent + '%';
                return;
            }
            
            // Atualizar barra verde imediatamente
            hpBar.style.width = currentHpPercent + '%';
            
            if (hpBarBg) {
                // Se o HP diminuiu, aplicar delay na barra vermelha
                if (currentHpPercent < this.lastHpPercent) {
                    setTimeout(() => {
                        hpBarBg.style.width = currentHpPercent + '%';
                    }, 500);
                } else {
                    // Se o HP aumentou (cura), atualizar imediatamente
                    hpBarBg.style.width = currentHpPercent + '%';
                }
            }
    ...
    updateUI(player, enemy, options = {}) {
        ...
        // Usar método especializado para HP
        this.updateHP(player);
        
        // Usar método especializado para Mana
        this.updateMana(player);
        
        const furiaBar = document.getElementById('p-furia-bar');
        if (furiaBar) furiaBar.style.width = Math.max(0, Math.min(100, player.furia || 0)) + '%';
        
        const xpBar = document.getElementById('p-xp-bar');
        if (xpBar && player.nextXp > 0) xpBar.style.width = (Math.max(0, player.xp || 0) / player.nextXp * 100) + '%';

        // Update XP display
        const xpCurrentEl = document.getElementById('p-xp-current');
        const xpNeededEl = document.getElementById('p-xp-needed');
        if (xpCurrentEl) xpCurrentEl.textContent = Math.floor(player.xp || 0);
        if (xpNeededEl) xpNeededEl.textContent = player.nextXp || 0;
        ...
        if (xpToNextEl) xpToNextEl.textContent = `${Math.ceil((player.nextXp || 0) - (player.xp || 0))} XP`;

        if (enemy) {
            // Usar método especializado para HP do inimigo
            this.updateEnemyHP(enemy);
            
            const enemyNameEl = document.getElementById('enemy-name');
            if (enemyNameEl) enemyNameEl.textContent = enemy.name;
            
            // Atualizar level do inimigo
            const enemyLevelEl = document.getElementById('enemy-level');
            if (enemyLevelEl) enemyLevelEl.textContent = `Lv.${enemy.lv || 1}`;
        ...
        document.getElementById('p-lv').textContent = player.lv || 1;
        document.getElementById('p-hp').textContent = Math.ceil(player.hp || 0);
        document.getElementById('p-max-hp').textContent = player.maxHp || 0;
        document.getElementById('p-mana').textContent = Math.ceil(player.mana || 0);
        document.getElementById('p-gold').textContent = player.gold || 0;
    ...
    closeAllModals() {
        document.getElementById('overlay').style.display = 'none';
        ['shop-modal', 'skill-modal', 'skill-detail-modal', 'death-modal', 'settings-modal', 'loot-modal', 'save-modal', 'skill-tree-modal'].forEach(id => {
    ...
    openSkillTreeModal() {
        ...
        // Inicializar a skill tree se ainda não foi inicializada
        if (!window.skillTree) {
            window.skillTree = new SkillTreeManager();
        }
        
        // Dar pontos iniciais ao player
        if (window.game && window.game.player) {
            const player = window.game.player;
            const skillPoints = player.level || 1;
            window.skillTree.setSkillPoints(skillPoints);
            
            // Atualizar display no modal
            const modalDisplay = document.getElementById('skill-points-modal');
            if (modalDisplay) {
                modalDisplay.textContent = skillPoints;
            }
        }
    }
    ...
    updateShopItemCards() {
        ...
        const gold = window.game.player.gold || 0;
        const prices = {
            // Itens existentes
            potion_hp: 28,
            mana_potion: 32,
