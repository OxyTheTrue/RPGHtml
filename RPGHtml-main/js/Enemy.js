...
export function createMonster(playerLv, difficultyMult = 1) {
    ...
    let finalHp = Math.max(6, Math.round(stats.baseHp * (1 + playerLv * cfg.hpScalePerLevel)));
    let finalDmg = Math.max(1, Math.round(stats.baseDmg * (1 + playerLv * cfg.dmgScalePerLevel)));
...
export function createMimic(playerLv, difficultyMult = 1) {
    ...
    const mult = Math.max(0.5, Math.min(2, difficultyMult));
    let finalHp = Math.max(8, Math.round(stats.baseHp * (1 + playerLv * cfg.hpScalePerLevel * cfg.mimicHpScale)));
    let finalDmg = Math.max(1, Math.round(stats.baseDmg * (1 + playerLv * cfg.dmgScalePerLevel * cfg.mimicDmgScale)));
    finalHp = Math.round(finalHp * mult);
    finalDmg = Math.round(finalDmg * mult);
    return {
        name: 'Mímico',
...
export function createThief(playerLv, difficultyMult = 1) {
    ...
    const mult = Math.max(0.5, Math.min(2, difficultyMult));
    let finalHp = Math.max(8, Math.round(stats.baseHp * (1 + playerLv * cfg.hpScalePerLevel)));
    let finalDmg = Math.max(1, Math.round(stats.baseDmg * (1 + playerLv * cfg.dmgScalePerLevel)));
    finalHp = Math.round(finalHp * mult);
    finalDmg = Math.round(finalDmg * mult);
    return {
...
export const SPECIAL_ENEMIES = {
    chest: () => {
        ...
        const finalHp = Math.floor(c.hpBase * (1 + c.hpPerLv));
        const finalDmg = Math.floor(c.dmgBase * (1 + c.dmgPerLv));
        return {
            name: 'Baú Misterioso',
            hp: finalHp,
            maxHp: finalHp,
